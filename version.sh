#!/bin/bash
# copyright ############################## #
# This file is part of the Xlhc package.   #
...
if [[ "$OSTYPE" == "darwin"* ]]
then
    sed -i '' "s/\(__version__ =\).*/\1 '"${new_ver}"'/"         xlhc/general.py
...
git reset
git add pyproject.toml xlhc/general.py tests/test_version.py
...
# Draft release
gh release create v${new_ver} --draft --generate-notes --title "Xlhc release ${new_ver}" --verify-tag
